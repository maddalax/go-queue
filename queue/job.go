...
func (job Job[T]) Ping(workerId string) {
	_, err := GetDatabase().NewUpdate().Model(&RawJob{}).Set("last_ping = ?, locked_by = ?", time.Now(), workerId).Where("id = ?", job.Id).Exec(context.Background())
	if err != nil {
		manager.onError <- JobError{
			error: err,
			job:   toJobForEvent(job),
